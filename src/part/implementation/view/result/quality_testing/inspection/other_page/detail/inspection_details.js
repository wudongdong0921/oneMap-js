...
import CardView from './inspection_details_card'
    ...
    render: function () {
        ...
        function getEblTreeAllInit() {
            ...
            _this.$api.getEblTreeAll({ zjxzflbId: "", cgid: _this.$data.cgxxbId }, function (res) {
                ...
                // if (treeSelectedLength > 0) {
                //     console.log('有数据，蓝色');
                //     _this.$el.find("#testing").css("background", "rgb(217,242,240)");
                // } else {
                //     _this.$el.find("#testing").css("background", "rgb(242,242,242)");
                // }
                //tree显示
                        ...
                        bindEvent(html, function (i) {
                            implementationDialog({
                                ...
                                params: {
                                    ...
                                    status: _this.$data.qcState //质检状态
                                },
                                title: '质检细则',
                                events: {},
                                onClose: function () {
        ...
        function setIn() {
            ...
            _this.$el.find('#zjbg').hide();
            _this.$api.getAchievementInfo({
                cgxxbId: _this.$data.cgxxbId,
            }, function (res) {
                console.log(res);
                _this.$el.find('#zjbg').click(()=>{
                    var _dialog = implementationDialog({
                        top: '45px',
                        width: '80%',
                        height: '90%',
                        path: '/affixFileView',
                        title: res.achievement.planningResultName+'.pdf',
                    });
    
                    setTimeout(function () {
                        _dialog.content.set(null, res.achievement.planningResultName+'.pdf',res.achievement.reportsAddressId);
                    }, 200);
                    // POBrowser.openWindowModeless(res.achievement.reportsAddressId, 'width=1824px;height=950px;');
                })
                // console.log(status);
                if(res.achievement.qcState == '2'){
                    _this.$el.find('#zjbg').show()
                }
        ...
        var handelFor = function (result,achievement) {
            ...
            obj.resultCodeNum = '质检文件名称：' +
                        ...
                        (achievement.errorSum==null?"":achievement.errorSum) + "个" + '<br/>' + '检查编码：' 
                        + num+  '<br/>' + '结果标识：' + jgbs
        ...
        _this.$el.find("#btn_start").click(function () {
            // console.log(treeSelectedLength);
