...
var AnalyzeResult = function (data, view, xzqh, syxIds,__OneMap) {
    ...
    this._html = $(
        ...
        '            <div>' +
        '                <div class="treeLineView" style="overflow:auto;height:250px;">' +
        ...
        '    </div>' +
        '</div>');
    var md = '50%';// 当禁用统计树或图标时，table列表设置的宽度
    ...
    //     this.echartView = new Echart(this._html.find('#analyze_pie')[0], _this.StaticDataAll.data, '3');
    // } else {
    //     // 统计图表不开启时，删除该元素 
    //     this._html.find('#analyze_pie').remove();
...
AnalyzeResult.prototype.initTable = function (params) {
    ...
    var thisselect = new icu.formElement.select({
        ...
        onChange: function () {
            thisselect.get((emsg, value) => {
                ...
                if ($("#analyze_pie")[0] !== null && $("#analyze_item_tree")[0] !== null) {
    ...
    this.views.$api.getTableDatas({
        ...
        _this._table = new icu.table({
            ...
            whereOptions: [],
            whereButtons: [],
            cellMinWidth: 80,//2021-06-01 陈薪名 修改bug HNXGTKJ-1748
            rightButtons: [
                {
                    class:'layui-btn Testo',// 2021-05-13 陈薪名 修改bug HNXGTKJ-1649
                    name: '导出', // 按钮显示文字
                    event: function(data){  
                        var url = config.InterfaceAddress.implementService + '/resourcesCarrying/dataTableExcel?syxId=' + _this.syxId + '&adcode=' + _this.xzqhs + '&fzckzdIdValue=' + _this.syxdj;
                        var name = '资源环境承载能力' + new Date($.ajax({async:false}).getResponseHeader("Date")).getTime();
            ...
            getEvent: function (data, setData) {
                _this.views.$api.getTableDatas({
                    ...
                    handelMJ(resTable.data.list,_this.selectType)
                    setData({
...
AnalyzeResult.prototype.initTreeLine = function (paramsData, type) {
    var _this = this;
    console.log(_this.StaticDataAll)
