...
RankingList.prototype.init = function () {
    ...
    if (this.option.data.length !== 0) {
        for (let i = 0; i < this.option.data.length; i++) {
            ...
            var itemHtml = $('<li><p id="indexP">' + itemData.index + '</p ><p>' + itemData.name + '</p><p>' + itemData.number + '</p></li > ').clone();
            if (itemData.index == 1) {
...
var layout = {
    ...
    styles: {
        ...
        'ranking-view .ranking-list li': {
            main: 'width: 100%; display: flex; justify-content: space-between; line-height: 37px; font-size: 18px;'
        },
