...
UploadFile.prototype.uploadFile = function (uploadLayout_ele) {
    var _this = this
    var updateUrl = config.InterfaceAddress.AnalysisServiceTest + '/layer/fileAnalysisReturnFileName'  //fileAnalysisReturnFileName
    var html = '';
    html += '   <form style="display:none;" action="' + updateUrl + '" method="post" enctype="multipart/form-data">';
    html += '       <input type="file" class="fileElement" accept=".geojson,.txt,.dwg,.zip" name="file" multiple="multiple"/>';
    ...
    form.ajaxForm({
        success: (result) => {
            ...
            if (result.code == 200) {
                ...
                this.event.getFileList(this.file)
                this.event.setCoordinate(_data, this.file,result)
            } else {
                uploadLayout_ele.find('.loadingBox').loading()
    ...
    form.find('.fileElement').change(function (e) {
        //_this.event.getFileList(e.target.files)
