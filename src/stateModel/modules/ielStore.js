...
ielStores.prototype.getUserAcodeList = function () {
    return this.$state.areacodeList;
}
ielStores.prototype.setData = function (key, data) {
    this.$state[key] = data; // 向共享数据中放值
}
ielStores.prototype.closePlayStatus = function () {// 关闭轮播状态
    this.$state.nextPlay = false;
    this.$state.autoPlay = false;
}
ielStores.prototype.startPlayStatus = function () {// 关闭轮播状态
    this.$state.nextPlay = true;
    this.$state.autoPlay = true;
}
ielStores.prototype.clearIntervalAuto = function () {//清除定时执行的方法
    clearInterval(this.$state.intervalAuto)
}
ielStores.prototype.handelDep = function () { // 处理市县级别
...
ielStores.prototype.handelPlaneData = function (data) {// 处理外层面板所需的数据
...
ielStores.prototype.isNeedZoom = function () {
    if ((this.$state.adCode.charAt(2) == '0' && this.$state.adCode.charAt(3) == '0') &&
...
ielStores.prototype.fnBuildColorLayerParam = function (colorAreas, areas) {//创建渲染颜色地图需要的数据
    for (let i = 0; i < colorAreas.length; i++) {
        ...
        for (let j = 0; j < areas.length; j++) {
            ...
            if (item.xzqhId == itemJ.properties.adcode) {
                ...
                item.color = this.fnCalcColor(parseFloat(item.score));
            }
        }
    }
    return colorAreas;
...
ielStores.prototype.fnCalcColor = function(value) {//地图颜色
    if (value < 60) {
