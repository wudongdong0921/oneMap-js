...
bdStores.prototype.getUserAcodeList = function () {
    return this.$state.areacodeList;
}
bdStores.prototype.setData = function (key, data) {
    this.$state[key] = data; // 向共享数据中放值
}
bdStores.prototype.closePlayStatus = function () {// 关闭轮播状态
    this.$state.nextPlay = false;
    this.$state.autoPlay = false;
}
bdStores.prototype.startPlayStatus = function () {// 关闭轮播状态
    this.$state.nextPlay = true;
    this.$state.autoPlay = true;
}
bdStores.prototype.clearIntervalAuto = function () {//清除定时执行的方法
    clearInterval(this.$state.intervalAuto)
}
bdStores.prototype.handelDep = function () { // 处理市县级别
...
bdStores.prototype.handelPlaneData = function (data) {// 处理外层面板所需的数据
...
bdStores.prototype.isNeedZoom = function () {
    if ((this.$state.adCode.charAt(2) == '0' && this.$state.adCode.charAt(3) == '0') &&
...
bdStores.prototype.fnBuildColorLayerParam = function (colorAreas, areas) {//创建渲染颜色地图需要的数据
